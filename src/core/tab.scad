...
/**
 * @brief Auto-tab logic.
 * @details Done this way so it can be used in `get_tab_angle`
 */
_auto_tab_angle = function()
    let(element = grid_element_current(true))
    grid_element_is_default(element) ? 90 // Center
 ...
 * @warning Only works inside `pattern_grid`
 */
 ...
 * @warning Auto only works when inside `pattern_grid`
 */
