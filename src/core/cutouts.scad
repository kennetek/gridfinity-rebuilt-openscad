...
module cut_compartment(size_mm) {
    ...
    difference() {
        rounded_square(size_mm, r_f2, center=true);
...
/**
 * @brief Maximum compartment size inside `pattern_grid`.
 * @warning Must be called from within `pattern_grid`.
 * @param height Optional.  passed throught to return a 3d vector.
 * @returns Max compartment [x, y] so there is `d_div` spacing between compartments.
 *          If height is passed in, [x, y, height].
 */
function auto_compartment_size(height = undef) =
    let(size_2d = grid_element_get_dimensions_mm(grid_element_current(true)))
    assert(
        size_2d.x > d_div/2 && size_2d.y > d_div/2,
        str("Grid Elements too small.  Must be at least d_div (",d_div,"mm).")
    )
    let(calculated=size_2d - [d_div/2, d_div/2])
    is_undef(height) ? calculated
    : concat(calculated, height);
 ...
 *                  Auto is not supported if called outside `pattern_grid`.
 * @param tab_top_left_only If the tab will only be on the top left compartment.
 *                          Only false is supported when called outside `pattern_grid`.
 * @param scoop 0.0-1.0 How much of a scoop should be present.
 */
module cut_compartment_auto(size_mm, style_tab, tab_top_left_only, scoop_percent) {
    element = grid_element_current(allow_default = true);
    is_top_left = grid_element_is_first_row(element) &&
                  grid_element_is_last_col(element);
